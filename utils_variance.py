...
def node_feature_selection_variance(n: pypsa.components.Network,
    ...
    if include_time:
        ...
        # dont resample because we want variance
        generator_capacity_factor = n.generators_t.p_max_pu[generator_names] #.resample(time_resolution).mean()
        #print(generator_capacity_factor)
        #generator_capacity_factor.index = pd.to_datetime(df.index)
        generator_capacity_factor = generator_capacity_factor.resample(interval).var() # variance over given interval
        #print(generator_capacity_factor)
...
def pypsa_to_pyg(n: pypsa.components.Network,
    ...
    # Feature selection
    features, feature_names = node_feature_selection_variance(n, carriers, include_time=include_time, time_resolution=time_resolution, interval=interval)
    #print(features)
